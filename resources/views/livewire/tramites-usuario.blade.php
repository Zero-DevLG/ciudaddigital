                ...
                @forelse ($tramites as $tramite)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-2 text-center space-x-2">
                            {{-- <a href="{{ route('tramites.ver', $tramite->id) }}"
                            ...
                            {{-- <form action="{{ route('tramites.eliminar', $tramite->id) }}" method="POST" class="inline-block"
                                  onsubmit="return confirm('¿Estás seguro de que deseas eliminar este trámite?');">
                                @csrf
                                @method('DELETE')
